...
describe("unwrapEvent", () => {
  it("returns a bare event unchanged", () => {
    const event: PlanEvent = {
      Action: "plan",
      ProjectS3Uri: "s3://bucket/project.zip",
...
describe("handler dispatch", () => {
  it("routes Action='plan' to the plan primitive", async () => {
    ...
    // Seed a fake project zip so the unzip step has something to chew on.
    s3.objects.set("s3://bucket/project.zip", makeMinimalProjectZip());
    ...
    expect(s3.ops.some((o) => o.kind === "download" && o.uri === "s3://bucket/project.zip")).toBe(
      true,
    );
  });
...
/**
 * Build the smallest valid zip `unzip(1)` accepts: a single empty file
 * inside an archive. We assemble it on disk via the system `zip` so the
 * test exercises the same code path the handler will hit in production,
 * rather than a hand-rolled zip blob that might parse on some unzip
 * builds and not others.
 */
function makeMinimalProjectZip(): Buffer {
  ...
  const dir = mk(join(tmpdir(), "hf-lambda-mkzip-"));
  try {
    wf(join(dir, "index.html"), "<!doctype html><title>test</title>");
    const zipPath = join(dir, "out.zip");
    const result = spawnSync("zip", ["-rq", zipPath, "."], { cwd: dir });
    if (result.status !== 0) {
...
async function makeMinimalPlanTar(): Promise<Buffer> {
  ...
  const result = spawnSync("tar", ["-czf", tarPath, "-C", dir, "plan.json", "meta"], {
    stdio: "inherit",
  });
  if (result.status !== 0) {
    throw new Error("makeMinimalPlanTar: tar failed");
  }
  return rf(tarPath);
}
