...
# Upload the project.
aws s3 cp my-project.zip "s3://${RENDER_BUCKET}/projects/my-project.zip"
...
aws stepfunctions start-execution \
  ...
  --input "$(cat <<EOF
{
  "ProjectS3Uri": "s3://${RENDER_BUCKET}/projects/my-project.zip",
...
```bash
# All defaults (mp4-h264-sdr fixture, chunk counts 2/4/8, PSNR >= 50 dB).
./scripts/smoke.sh
...
`jq`, `zip`. The script defaults to `--profile
engineering-767398024897` (the HeyGen engineering account); override
via `--profile <name>` or `AWS_PROFILE=<name>`.
